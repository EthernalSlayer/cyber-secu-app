...
const RedPill = () => {
  return (
    <div>
      ...
      <ShellText>
        Mainteant que tu connais tout les secrets des malwares qui t'entoure que
        dirait tu d'apprendre a mieux connaître les differentes techniques
        utilisées par les <Important>h4ck3rs</Important> ?
      </ShellText>
      <ShellText>
        Donc sur cette page tu apprendras se qu'est une attaque, les différents
        types d' attaques et toutes les méthode qui leur sont associées.
      </ShellText>
      <ShellTitle>Une attaque qu'est ce que c'est ?</ShellTitle>
      <ShellText>
        Sur cette page quand l'on va parler d'attaque on parlera en faite de
        ...
        essayerons d'entrer un peu plus dans les détails pour celle là.
      </ShellText>
      ...
      <ShellText>
        ...
        <Italic>distributed denial of service</Italic>) vise a saturer de
        requêtes un site pour qu'il ne soit plus en mesure de répondre. Dans les
        faits il faut une armée d'ordinateur pour accomplir une telle attaque
        mais c'est là que les notions aborder dans la page précédente vont te
        servir. Tu te rappel que certains malware permettent aux hackers de
        prendre le contrôle de ta machine ou d'utiliser ses ressources et bien
        grâce a cela le hacker va se créer une armée d'ordinnateur dit{" "}
        ...
        aussi sous le nom <Important>"botnet"</Important>) jusqu'a exploser le
        nombre de requête autoriser du serveur ou saturer sa bande passante.
      </ShellText>
      <Image
        src={ddos2}
      ...
      <ShellText>
        ...
        <Italic>operation PayBack</Italic> dont le but était de se venger des
        adversaires de l'internet libre notament en attaquant Mastercard, Visa
        et paypal qui avait priver de leur services le site{" "}
        <Important>"wikileaks"</Important> qui ne pouvait alors plus recevoir de
        donation. Certains parle de cette operation comme la première cyber
        guerre. Dans ce cas précis les <Important>DDOS</Important> n'ont pas
        utilisé d'ordinateur zombie, Anonymous a créer un programme qui s'appel{" "}
        <Important>LOIC</Important> (<Italic>Low Orbit Ion Canon</Italic>) qui
        est très facile d'utilisation et permettait a chaque partisan du
        mouvement de pouvoir participer aux attaques DDOS sans aucune
        connaissance du hack.
      </ShellText>
      ...
      <ShellText>
        <Italic>
          ...
          tant que dev web donc je t'invite a te renseigner sur différents types
          de DDOS:
        </Italic>
      </ShellText>
      <ul>
      ...
      <ShellText>
        ...
        globale (<Italic>un schéma d'attaque</Italic>) plutot qu'un type
        d'attaque précis. Mais comprendre son fonctionement t'aidera a mieux
        comprendre plusieurs autres attaques dont nous parlerons plus tard. Le
        principe de base est simple et donc facile a expliquer le but du hacker
        est de se placer entre deux parties, par exemple entre un utilisateur et
        un site internet, et de faire transiter l'échange par lui en se fesant
        passer pour le site internet auprès de l'utilisateur et en se fesant
        ...
        possibilitées que nous aborderons en détails lorsque nous parlerons
        d'autres attaques.
      </ShellText>
      <Image
        src={mitm}
      ...
      <ShellText>
        ...
        nom d'utilisateur et mot de passe, le serveur vérifie si un compte
        ...
        utiliser votre compte a votre place. C'est cela que l'on appel le{" "}
        ...
        dérouler de l'opération. Le hacker va se placer entre vous et le site
        internet. Il va détourner le traffic par exemple en vous fesant croire
        ...
        vos info qu'il redirigera vers le site internet et au moment où le site
        vous enverra le fameux cookie le hacker le recevra avant vous et pourra
        donc être connecté a votre session avec les mêmes droits que vous même.
      </ShellText>
      <ShellText>
        <Italic>
      ...
      <ShellText>
        ...
        certains l'appel <Important>ARP Poisoning</Important> d'autres l'appel{" "}
        <Important>ARP Spoofing</Important> mais il s'agit exactement de la même
        chose. L ' <Important>ARP Spoofing</Important> permet à l'attaquant de
        détourner des flux de communications transitant entre une machine cible
        et une passerelle : routeur, box, etc. L'attaquant peut ensuite écouter,
        modifier ou encore bloquer les paquets réseaux. Le hacker doit pour cela
        être connecté au même réseau que la victime (un wifi public par
        exemple). Cela permet à l'attaquant d'écouter, de corrompre, de couper
        l'accès réseau mais aussi d'usurper une adresse IP ou de bloquer le
        trafic. Donc pour reprendre l'exemple précédent l'attaque aurrait put se
        dérouler ainsi, dans un premier hack de votre <Italic>wifi</Italic> ou
        connexion a un <Italic>wifi public</Italic>, dans un second utilisation
        de l'<Important>ARP Spoofing</Important> et enfin le{" "}
      ...
      <ShellText>
        ...
        monde en entend parler, tout le monde c'est se que c'est, mais en tant
        que dev web, un jour tes bases de données pourraient être victime d'une
        fuite de données suite a une attaque. Et tu seras alors bien content
        d'avoir revue tout sa avec moi pour pouvoir faire une belle campagne
        ant-phishing à envoyer à tout tes clients. Il s'agit plus d'un piège
        ...
        arnaque (de préférence a grande échelle pour multiplier les chances de
        trouver une victime crédule ou même inatentive) qui vise à récupérer les
        données confidentielles d'un internaute. Le pirate va envoyer un mail
        aux victime en se fesant passer pour une entreprise chez qui vous avez
        un compte. Dans ce mail vous trouverez un lien vous renvoyant vers une
        copie très bien faite d'une page de connexion de l'entreprise pour qui
        il essaie de se faire passer.{" "}
        <Italic>
          Et si par malheur vous entrer vos identifiants et mot de passe sur ces
          fausses pages ils atteriront directement entre les mains du hacker
        </Italic>
        .
      </ShellText>
      <Image
      ...
      <ShellText>
        ...
        toutes fonctionne sur le même modèle comportant une application
        interagissant avec une bases de données. Commençons avec un exemple
        ...
        pour faire simple au lieu de chercher a deviner le mot de passe, vous
        envoyer un petit bout de code qui sera interprétés comme valide. Il
        existe des outils permettant d'automatiser ce type d'attaque (notamment
        le logiciel iranien <Important>"Havij"</Important>) car les taper a la
        main est assez fastidieux et plus compliquer aussi. Mais ces outils
        servent aussi bien aux devs voulant vérifier la sécuriter de leur
        application qu'aux scripts kiddies voulant jouer aux hacker, car il
      ...
      <ShellText>
        ...
        peux te renseigner sur ces différentes méthode d'injections:
      </ShellText>
      <ul>
        <li>
      ...
      <ShellText>
        <Italic>
          Une manière simple d'éviter se genre d'attaque est l'échappement des
      ...
      <ShellText>
        ...
        d'analyser les <Italic>paquets</Italic> récupérer pour déchiffrer leur
        contenu à l'aide d'autres logiciels. Pour mieux comprendre les liaisons
        qu'il existe entre les différentes méthodes je vais reprendre notre
        example utiliser pour l'attaque <Important>Man In The Middle</Important>
        ...
        possibilitées s'offre a moi je suis patient et j'attend qu'une nouvelle
        connexion s'établisse ou je déconnecte un utilisateur pour le forcer a
        se reconnecter et maintenant il ne me reste plus qu'a récupérer le{" "}
      ...
      <ShellText>
        <Italic>
          ...
          simplement de rester a jour par exemple pour les box internet sachez
          que de nos jours elles utilises toute le protocole de sécurité WPA2
          qui est beaucoup plus dur a cracker que son ancètre le WEP. De même le
          protocole HTTP n'est presque plus utiliser car il fesait transiter les
          données en "clair" (non chiffrer) contrairement aux HTTPS qui est
      ...
      <ShellText>
        ...
        pour que le naviguateur de la ou des victimes interpète ce code. Il peut
        ...
        sites. Les possibilitées des <Important>attaques XSS</Important> sont
        très large puisque le pirate peut utiliser tous les languages supportés
        par le naviguateur (
        <Italic>
          HTML, CSS, mais bon le plus interressant reste le JavaScript
        </Italic>
        ) afin d'éxécuter du code qu'il injecte dans le site grâce a ces
        ...
        messages et commentaires d'autres utilisateur. Pour expliquer simplement
        cette attaque prenons l'exemple d'un petit rigolo (
      ...
      <ShellText>
        Celle ci est la plus simple a comprendre, le pirate va envoyer un code
        malicieux dans une application web qui va le stocker (par exemple dans
        sa base de donnée). Ensuite le contenu malicieux sera retourné dans le
        navigateur des autres utilisateurs a chaque fois qu'ils iront sur le
        site.
      </ShellText>
      <ShellText>
      ...
      <ShellText>
        Ce deuxième type d'attaque XSS ne stock pas le contenu malicieux sur le
        serveur web. Le contenu malicieux est par exemple livré à la victime via
        un url qui le contient. Et donc le code malicieux ne sera exécuter que
      ...
      <ShellText>
        <Italic>
          ...
          et échapper tout les caractère spéciaux et vérifier que le contenu est
